...
public class Pawn extends Piece {
    ...
    public List<Move> possibleMoves(Board board) {
        ...
        if (y == 1 && board.getSquare(x, y + 2 * coef) == null) {
            if (addMove(this, x, y, x, y + 2 * coef, board, possibleMoves)) {
                canBeTakenByEnPassant = true; // Set en passant flag if pawn moves two squares forward
            }
        }
        
        // Normal move forward
        if (board.getSquare(x, y + 1 * coef) == null) addMove(this, x, y, x, y + 1 * coef, board, possibleMoves);

        // Capture moves diagonally
        if (board.getSquare(x - 1, y + 1 * coef) != null) addMove(this, x, y, x - 1, y + 1 * coef, board, possibleMoves);
        if (board.getSquare(x + 1, y + 1 * coef) != null) addMove(this, x, y, x + 1, y + 1 * coef, board, possibleMoves);
