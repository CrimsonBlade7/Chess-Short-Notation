...
```powershell
javac -d out (Get-ChildItem -Recurse src\main -Filter *.java | Where-Object { -not $_.PSIsContainer }).FullName
...
```powershell
javac -cp lib\junit-platform-console-standalone-1.9.3.jar -d out_test (Get-ChildItem -Recurse src\main,src\test -Filter *.java | Where-Object { -not $_.PSIsContainer }).FullName
